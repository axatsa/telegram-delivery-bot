...
class TelegramBotNotificationIntegration(IntegrationTelegramBase, ClientBase):
    ...
    def _create_error_response(self, error_code: int, description: str) -> IntegrationErrorResponse:
    ...
    async def _fetch_settings(self, cache_key: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            logger.error(f"Error fetching settings for ID {self.connected_integration_id}: {error}")
            raise
    ...
    async def _add_subscriber(self, chat_id: str) -> None:
        ...
        try:
            ...
            if chat_id not in subscribers:
                ...
                logger.info(f"Added subscriber {chat_id} for ID {self.connected_integration_id}")
        except Exception as error:
    ...
    async def _remove_subscriber(self, chat_id: str) -> None:
        ...
        try:
            ...
            if chat_id in subscribers:
                ...
                if settings.redis_enabled and redis_client:
                    await redis_client.delete(cache_key)
                logger.info(f"Removed subscriber {chat_id} for ID {self.connected_integration_id}")
        except Exception as error:
    ...
    async def _initialize_bot(self) -> None:
        ...
        if self.bot:
            return
        settings_map = await self._fetch_settings(f"clients:settings:telegram:{self.connected_integration_id}")
        bot_token = settings_map.get(TelegramSettings.BOT_TOKEN.value.lower())
        if not bot_token:
            raise ValueError(f"Bot token not found for ID {self.connected_integration_id}")
        self.bot = Bot(
            token=bot_token,
    ...
    async def _setup_handlers(self) -> None:
        ...
        async def handle_start_command(message: types.Message):
            ...
            try:
                ...
                await message.answer("You are now subscribed to cheque and session notifications!")
                logger.info(f"Client {chat_id} subscribed for ID {self.connected_integration_id}")
            except Exception:
        ...
        async def handle_stop_command(message: types.Message):
            ...
            try:
                ...
                logger.info(f"Client {chat_id} unsubscribed for ID {self.connected_integration_id}")
            except Exception:
        ...
        async def handle_cheque_details(callback_query: types.CallbackQuery):
            ...
            try:
                async with RegosAPI(self.connected_integration_id) as api:
                    ...
                    if not cheques:
                        ...
                        return
                    cheque = cheques[0]
                    operations = await api.docs.cheque_operation.get(DocChequeOperationGetRequest(doc_sale_uuid=uuid))
                    payments   = await api.docs.retail_payment.get(DocRetailPaymentGetRequest(doc_sale_uuid=uuid))
            except Exception as error:
            ...
            message_text = format_cheque_details(cheque=cheque, operations=operations, payments=payments)
        ...
        async def handle_session_details(callback_query: types.CallbackQuery):
                ...
                async with RegosAPI(self.connected_integration_id) as api:
                    ...
                    operations = await api.docs.cash_operation.get_amount_details(
                        CashAmountDetailsGetRequest(
                            start_date=session.start_date, end_date=session.close_date, operating_cash_id=session.operating_cash_id
                        )
                    )
                    counts = await api.reports.retail_report.get_counts(
                        CountsGetRequest(
                            start_date=session.start_date, end_date=session.close_date, operating_cash_ids=[session.operating_cash_id]
                        )
                    )
            ...
            message_text = format_session_details(session=session, operations=operations, counts=counts, payments=payments)
    ...
    @retry(
        ...
        before_sleep=lambda retry_state: logger.debug(f"Retry attempt: {retry_state.attempt_number}")
    )
    async def connect(self, data: Optional[Dict] = None, **kwargs) -> Dict:
        """Connect to Telegram and set up the webhook."""
        logger.info(f"Connecting to TelegramBotNotificationIntegration (ID: {self.connected_integration_id})")
        cache_key = f"clients:settings:telegram:{self.connected_integration_id}"
        try:
    ...
    async def disconnect(self, **kwargs) -> Dict:
        ...
        logger.info(f"Disconnecting from TelegramBotNotificationIntegration (ID: {self.connected_integration_id})")
        if not self.bot:
    ...
    async def reconnect(self, **kwargs) -> Dict:
        ...
        logger.info(f"Reconnecting to TelegramBotNotificationIntegration (ID: {self.connected_integration_id})")
    ...
    async def handle_webhook(self, action: Optional[str] = None, data: Optional[Dict] = None, **kwargs) -> Dict:
        ...
        supported_actions = {"DocSessionOpened", "DocSessionClosed", "DocChequeClosed", "DocChequeCanceled"}
        ...
        try:
            if webhook_action in {"DocChequeClosed", "DocChequeCanceled"}:
                ...
                if not cheques:
                    ...
                    cheque = cheques[0]
                    message_text = format_cheque_notification(cheque=cheque, action=webhook_action)
                    # Create details button
                    keyboard = InlineKeyboardMarkup(inline_keyboard=[
                        [InlineKeyboardButton(text="Детали чека", callback_data=f"cdetails_{uuid}_{webhook_action}")]
                    ])
                ...
                if not sessions:
                    ...
                    session = sessions[0]
                    message_text = format_session_notification(session=session, action=webhook_action)
                    if session.closed:
                        # Create details button
                        keyboard = InlineKeyboardMarkup(inline_keyboard=[
                            [InlineKeyboardButton(text="Детали смены", callback_data=f"sdetails_{uuid}_{webhook_action}")]
                        ])
            else:
        ...
        for chat_id in subscribers:
            try:
                await self.bot.send_message(chat_id=chat_id, text=message_text, reply_markup=keyboard if keyboard else None)
    ...
    async def handle_external(self, envelope: Dict) -> Dict:
        ...
        if not isinstance(payload, dict):
            return self._create_error_response(400, "Invalid request body: JSON object expected").dict()
        ...
        try:
            ...
            return self._create_error_response(500, "Error processing Telegram update").dict()
